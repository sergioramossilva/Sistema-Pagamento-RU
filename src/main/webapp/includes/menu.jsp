<!-- Fixed navbar -->
<div class="navbar navbar-default navbar-fixed-top" role="navigation">
	<div class="container">
		<div class="navbar-header">
			<button type="button" class="navbar-toggle" data-toggle="collapse"
				data-target=".navbar-collapse">
				<span class="sr-only">Toggle navigation</span> <span
					class="icon-bar"></span> <span class="icon-bar"></span> <span
					class="icon-bar"></span>
			</button>
			<a class="navbar-brand" href="#">ModeloWEB</a>
		</div>
		<div class="collapse navbar-collapse">
			<ul class="nav navbar-nav">
				<li class="active"><a href="#">Início</a></li>

				<li class="dropdown"><a href="#" class="dropdown-toggle"
					data-toggle="dropdown">Cadastros <b class="caret"></b></a>
					<ul class="dropdown-menu">
						<li class="dropdown-header">Principais</li>
						<li><a href="#">Aluno</a></li>
						<li class="divider"></li>
						<li class="dropdown-header">Complementares</li>
						<li><a href="#">Cidade</a></li>
						<li><a href="#">Estado</a></li>
					</ul></li>

				<li class="dropdown"><a href="#" class="dropdown-toggle"
					data-toggle="dropdown">Movimentos <b class="caret"></b></a>
					<ul class="dropdown-menu">
						<li class="dropdown-header">Principais</li>
						<li><a href="#">Exemplo</a></li>
						<li class="divider"></li>
						<li class="dropdown-header">Complementares</li>
						<li><a href="#">Exemplo</a></li>
						<li><a href="#">Exemplo</a></li>
					</ul></li>

				<li class="dropdown"><a href="#" class="dropdown-toggle"
					data-toggle="dropdown">Relatórios <b class="caret"></b></a>
					<ul class="dropdown-menu">
						<li class="dropdown-header">Gerenciais</li>
						<li><a href="#">Exemplo</a></li>
						<li><a href="#">Exemplo</a></li>
						<li class="divider"></li>
						<li class="dropdown-header">Operacionais</li>
						<li><a href="#">Exemplo</a></li>
						<li><a href="#">Exemplo</a></li>
					</ul></li>

				<li><a href="#">Ajuda</a></li>
				<li><a href="#">Sair</a></li>
			</ul>
		</div>
		<!--/.nav-collapse -->
	</div>
</div>